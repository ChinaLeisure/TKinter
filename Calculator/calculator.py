...
result2 = tkinter.StringVar() #显示面板2，用于显示计算过程
result2.set('')

#显示版
label = tkinter.Label(root, font=('微软雅黑',20),bg='#EEE9E9',bd='9',fg='#828282',anchor='se',textvariable=result2)
label.place(width=280,height=170)
label2=tkinter.Label(root,font=('微软雅黑',30),bg='#EEE9E9',bd='9',fg='black',anchor='se',textvariab=result)
...
btnac = tkinter.Button(root,text='AC',font=('黑体',20),fg='orange',bd=0.5,command=lambda:pressCompute('AC'))
btnac.place(x=0,y=230,width=70,height=55)

btnback = tkinter.Button(root,text='←',font=('微软雅黑',20),bd=0.5,fg=('#4F4F4F'),command=lambda:pressCompute('b'))
btnback.place(x=70,y=230,width=70,height=55)

btndivi = tkinter.Button(root,text='÷',font=('微软雅黑',20),bd=0.5,command=lambda:pressCompute('/'))
btndivi.place(x=140,y=230,width=70,height=55)

btnmul = tkinter.Button(root,text='×',font=('微软雅黑',20),bd=0.5,fg=('#4F4F4F'),command=lambda:pressCompute('*'))
btnmul.place(x=210,y=230,width=70,height=55)

btnsub = tkinter.Button(root,text='-',font=('微软雅黑',20),bd=0.5,fg=('#4F4F4F'),command=lambda:pressCompute('-'))
btnsub.place(x=210,y=285,width=70,height=55)

btnadd = tkinter.Button(root,text='+',font=('微软雅黑',20),bd=0.5,fg=('#4F4F4F'),command=lambda:pressCompute('+'))
btnadd.place(x=210,y=340,width=70,height=55)

btnequ = tkinter.Button(root,text='=',font=('微软雅黑',20),bg='orange',bd=0.5,fg=('#4F4F4F'),command=lambda:pressEqual())
btnequ.place(x=210,y=395,width=70,height=110)

btnper = tkinter.Button(root,text='%',font=('微软雅黑',20),bd=0.5,fg=('#4F4F4F'),command=lambda:pressCompute('%'))
btnper.place(x=0,y=450,width=70,height=55)

btnper = tkinter.Button(root,text='.',font=('微软雅黑',20),bd=0.5,fg=('#4F4F4F'),command=lambda:pressCompute('%'))
btnper.place(x=140,y=450,width=70,height=55)
...
#数字函数
def pressNum(num): #设置一个数字函数  判断是否按下数字  并获取数字将数字写在显示板上
    ...
    if isPressSign == False:
        ...
        result.set(0)
        isPressSign = False #重新将运算符的状态设置为否

    #判断界面数字是否为0
    oldnum = result.get() #第一步
    if oldnum=='0': #如果界面数字为0  则获取按下的数字
        result.set(num)
...
def pressCompute(sign):
    ...
    lists.append(num)  #保存界面获取的数字到列表

    lists.append(sign) #将按下的运算符号保存到列表中
    isPressSign = True

    if sign =="AC":     #如果按下的键位是AC，则清空列表的内容，将屏幕上的数字键设置为默认数字0
        lists.clear()
        result.set(0)

    if sign =='b':  #如果按下的退格--》，则获取当前数字第一位到倒数第二位
        a = num[0:-1]
        list.clear()
        result.set(a)
...
def pressEqual():
    ...
    computrStr = ''.join(lists) #将列表内容用jion命令将字符串链接起来
    endNum = eval(computrStr)   #用eval命令运算字符串中内容

    result.set(endNum)          #将运算结果显示到屏幕一上
